(ns hkma.main
  "Kotodama WASM entrypoint for the Hkma clean-room actor (L4) — Clojure port.
  ...
  facts (`hkma.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"hkma.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hkma-compat" :tier tier :entities entities} 200])
